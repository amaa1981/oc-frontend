  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <!-- <el-form-item :label="$t('dictInfo.dictLabel')" prop="dictLabel">
        <el-input v-model="queryParams.dictLabel" :placeholder="$t('commonTips.pleaseEnter')" clearable
      ...
      <el-form-item :label="$t('dictInfo.status')" prop="status">
        <el-select v-model="queryParams.status" :placeholder="$t('commonTips.pleaseSelect')" clearable>
          <el-option :label="$t('algorithm.statusNormal')" value="0" />
          <el-option :label="$t('algorithm.statusDisabled')" value="1" />
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['system:dict:add']">{{ $t('commonBtn.add') }}</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['system:dict:edit']">{{ $t('commonBtn.edit') }}</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['system:dict:remove']">{{ $t('commonBtn.remove') }}</el-button>
      </el-col>
      <!-- <el-col :span="1.5">
        <el-button
          type="warning"
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column :label="$t('dictInfo.dictLabel')" align="center" prop="dictLabel">
        ...
        <template slot-scope="scope">
          {{ $t("dict.v1_alarm_type." + scope.row.dictValue) }}
        </template>
      ...
      <el-table-column :label="$t('dictInfo.status')" align="center" prop="status">
        <template slot-scope="scope">
          <el-tag :type="scope.row.status == '0' ? 'success' : 'danger'">
            {{ scope.row.status == '0' ? $t('algorithm.statusNormal') : $t('algorithm.statusDisabled') }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('commonBtn.operate')" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" class="btn-edit" @click="handleUpdate(scope.row)"
            v-hasPermi="['system:dict:edit']">{{ $t('commonBtn.edit') }}</el-button>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item :label="$t('dictInfo.status')" prop="status">
          <el-radio-group v-model="form.status">
            <el-radio label="0">{{ $t('algorithm.statusNormal') }}</el-radio>
            <el-radio label="1">{{ $t('algorithm.statusDisabled') }}</el-radio>
          </el-radio-group>
        </el-form-item>
...
export default {
  ...
  created() {
    // Fixed dictType to v1_alarm_type
    this.queryParams.dictType = "v1_alarm_type";
    this.defaultDictType = "v1_alarm_type";
    this.getList();
  },
  methods: {
