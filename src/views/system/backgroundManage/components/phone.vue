...
export default {
    ...
    return {
      ...
      rules: {
        phone: [
          ...
          { pattern: /^\d{11}$/, message: this.$t("push.phoneFormatError"), trigger: "blur" },
        ],
        status: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listPhonePush(this.queryParams).then((response) => {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = this.$t("push.addPhonePush");
    },
    handleUpdate(row) {
      this.reset();
      ...
      getPhonePush(id).then((response) => {
        ...
        this.open = true;
        this.title = this.$t("push.editPhonePush");
      });
    },
    submitForm() {
