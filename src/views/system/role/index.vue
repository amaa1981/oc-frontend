  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item :label="$t('role.createTime')">
        <el-date-picker
          ...
          type="daterange"
          range-separator="-"
          :start-placeholder="$t('role.startTime')"
          :end-placeholder="$t('role.endTime')"
        ></el-date-picker>
      </el-form-item>
      <el-form-item>
        <el-button
          ...
          >
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item
                ...
                v-hasPermi="['system:role:edit']"
                >{{ $t("role.dataPermi") }}</el-dropdown-item
              >
              <el-dropdown-item
                command="handleAuthUser"
                icon="el-icon-user"
                v-hasPermi="['system:role:edit']"
                >{{ $t("role.authUser") }}</el-dropdown-item
              >
            </el-dropdown-menu>
          </el-dropdown>
        </template>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item :label="$t('role.menuPermi')">
          <el-checkbox
            v-model="menuExpand"
            @change="handleCheckedTreeExpand($event, 'menu')"
            >{{ $t("role.menuExpand") }}</el-checkbox
          >
          <el-checkbox
            v-model="menuNodeAll"
            @change="handleCheckedTreeNodeAll($event, 'menu')"
            >{{ $t("role.menuNodeAll") }}</el-checkbox
          >
          <el-checkbox
            v-model="form.menuCheckStrictly"
            @change="handleCheckedTreeConnect($event, 'menu')"
            >{{ $t("role.menuCheckStrictly") }}</el-checkbox
          >
          <el-tree
            class="tree-border"
            :data="menuOptions"
  ...
  data() {
    return {
      ...
      dataScopeOptions: [
        {
          value: "1",
          label: this.$t("role.dataScopeOptions1"),
        },
        {
          value: "2",
          label: this.$t("role.dataScopeOptions2"),
        },
        {
          value: "3",
          label: this.$t("role.dataScopeOptions3"),
        },
        {
          value: "4",
          label: this.$t("role.dataScopeOptions4"),
        },
        {
          value: "5",
  ...
  methods: {
    ...
    handleStatusChange(row) {
      let text =
        row.status === "0"
          ? this.$t("role.enable")
          : this.$t("role.deactivate");
      this.$modal
        .confirm(
          this.$t("role.confirmChangeStatus", { text, username: row.roleName })
        )
        .then(function () {
          return changeRoleStatus(row.roleId, row.status);
        })
        .then(() => {
          this.$modal.msgSuccess(this.$t("role.success", { text }));
        })
        .catch(function () {
    ...
    handleAdd() {
      ...
      this.open = true;
      this.title = this.$t("role.dialogTitle_a");
    },
    /** Edit handler */
    handleUpdate(row) {
      this.reset();
    ...
    /** Role permission scope change handler */
    dataScopeSelectChange(value) {
      if (value !== "2") {
        this.$refs.dept.setCheckedKeys([]);
      }
    },
    /** Assign data permission handler */
    handleDataScope(row) {
      this.reset();
      const deptTreeSelect = this.getDeptTree(row.roleId);
      console.log(deptTreeSelect);
      getRole(row.roleId).then((response) => {
        ...
        this.title = this.$t("role.authTitle");
      });
    },
    /** Assign user handler */
    ...
    /** Submit data permission handler */
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm(this.$t("role.confirmRemove", { roleIds }))
        .then(function () {
